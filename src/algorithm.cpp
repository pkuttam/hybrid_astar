...
Node3D* Algorithm::hybridAStar(Node3D& start,
  ...
  O.push(&start);
  iPred = start.setIdx(width, height); // index of predessor
  nodes3D[iPred] = start; // node list set value of the grid loc which is start

  // NODE POINTER
  Node3D* nPred; //predessor node 
  Node3D* nSucc; //successor node
  ...
  while (!O.empty()) {
    ...
    nPred = O.top(); // get lowest cost predessor
    // set index
